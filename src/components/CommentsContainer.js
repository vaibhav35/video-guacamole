      ...
      {comments?.items?.length !== 0 ? (
        <div className="">
          {comments?.items?.map((comment) => {
            ...
            const { textDisplay,likeCount, textOriginal, authorProfileImageUrl, authorDisplayName ,authorChannelUrl} = commentSnippet?.topLevelComment.snippet;
            return (
            
            <div className="block">
            <div className="flex px-2 py-3 border-b-2  ">
              <div className=" flex-shrink-0 w-11"><img className="rounded-full w-full" src={authorProfileImageUrl} alt=" "  /></div>
